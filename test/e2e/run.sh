...
run_e2e_tests() {
  if [ "${E2E_JSON}" == "true" ]
  then
    go test -v -timeout=6h -tags="$E2E_TAGS" -p=1 --json github.com/devendra/es-cloud-on-k8s/v2/test/e2e/... "$@"
  else
    go test -v -timeout=6h -tags="$E2E_TAGS" -p=1 github.com/devendra/es-cloud-on-k8s/v2/test/e2e/... "$@"
  fi
