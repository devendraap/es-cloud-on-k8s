...
|webhook-name         | elastic-webhook.k8s.acceldata.io | Name of the `ValidatingWebhookConfiguration` resource.
...
To disable the webhook, set the <<{p}-operator-config, `enable-webhook`>> operator configuration flag to `false` and remove the `ValidatingWebhookConfiguration` named `elastic-webhook.k8s.acceldata.io`:

[source,sh]
----
kubectl delete validatingwebhookconfigurations.admissionregistration.k8s.io elastic-webhook.k8s.acceldata.io
----
...
....
admission webhook "elastic-es-validation-v1.k8s.acceldata.io" denied the request: Elasticsearch.elasticsearch.k8s.acceldata.io "quickstart" is invalid: some-misspelled-field: Invalid value: "some-misspelled-field": some-misspelled-field field found in the kubectl.kubernetes.io/last-applied-configuration annotation is unknown
....
