...
[source,sh]
----
# If you installed using the Helm defaults, the name of the webhook would be elastic-operator.elastic-system.k8s.acceldata.io
# If the operator name or namespace was changed during the installation, the name would reflect those changes.
WEBHOOK=$(kubectl get validatingwebhookconfiguration --no-headers -o custom-columns=NAME:.metadata.name | grep 'k8s.acceldata.io')
kubectl patch validatingwebhookconfiguration "$WEBHOOK" \
    --patch='{"webhooks": [{"name": "elastic-es-validation-v1.k8s.acceldata.io", "matchPolicy": "Exact"}, {"name": "elastic-es-validation-v1beta1.k8s.acceldata.io", "matchPolicy": "Exact"}]}'
----
